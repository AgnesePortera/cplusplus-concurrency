...
### std::shared_futures
Sometimes there is the need to have more than one thread waiting on the same future object.
If you try to share the same *std::promise** (see the following example), an exception for invalid futures will be thrown when the second thread try to call the same *get()* function.
...
One possible solution is to use a **std::shared_future**.

> [!IMPORTANT]
> **Be carefull when using promises because future are one off events. After one thread executes future.get() or future.wait() functions, the future will go in an invalid state.**

> [!NOTE]
> **If multiple threads going to call future.get(), shared_future objects must be used.**
